...
from .crossmodal import CrossModalMamba  # 新增导入
# 导入GSG模块
from .GSG import GroundingModule  # 新增导入

class Fusion_VQA(nn.Module):
    def __init__(self,
                 text_encoder=None,
                 text_decoder=None,
                 tokenizer=None,
        ...
        self.visual_encoder = VisionTransformer(
            img_size=config['image_res'], patch_size=16, embed_dim=config['hidden_dim'], depth=12, num_heads=12,
        ...
        self.text_decoder = BertLMHeadModel.from_pretrained(text_decoder, config=config_decoder)
         # 新增Mamba跨模态融合层
        self.cross_mamba = CrossModalMamba(
            dim=config['hidden_dim'], 
            proj_dim=config['proj_dim']
        )
        # 修改ITM分类头输入维度
        self.itm_head = nn.Linear(config['hidden_dim'], 2)  # 直接线性分类

        # 新增GSG模块
        self.grounding_module = GroundingModule(d_model=config['hidden_dim'])  # 新增
    ...
    def forward(self, image, question, answer=None, alpha=0, k=None, train=True):
        ...
        # question_embeds = self.text_encoder(question.input_ids,
        #                                     attention_mask=question.attention_mask).last_hidden_state
        # 文本编码（传入图像编码作为交叉注意力输入）
        question_output = self.text_encoder(
            input_ids=question.input_ids,
            attention_mask=question.attention_mask,
            encoder_hidden_states=image_embeds,       # 关键：图像编码作为交叉注意力输入
            encoder_attention_mask=image_atts,         # 图像注意力掩码（全1）
            return_dict=True                           # 确保返回字典格式
        )
        question_embeds = question_output.last_hidden_state

        # ========== 关键修改：Mamba跨模态融合 ==========
        fused_feature = self.cross_mamba(image_embeds, question_embeds)  # [B, 768]
        # 分类层
        itm_logits = self.itm_head(fused_feature)  # [B, 2]
    ...
    def rank_answer(self, question_states, question_atts, answer_ids, answer_atts, k):
        ...
        # print(f"prob_first_token shape: {prob_first_token.shape}")
        # 在model_vqa.py的第174行附近添加
        max_k = prob_first_token.size(1)  # 获取维度1的大小
        k = min(k, max_k)  # 确保k不超过最大可能值
