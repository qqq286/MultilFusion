...
def initialize_clip_video(config, num_patches=240, T=8, use_checkpoint=False):
    from models import clip_video
    if config["clip_name"] == "ViT-B-16":
        clip_model, preprocess = clip_video.load(model_path="ViT-B-16.tar", jit=False, img_size=config['image_res'], T=T, 
