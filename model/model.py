...
from .crossmodal  import crossmodal  # 新增导入
...
class FusionBASE(nn.Module):
    def __init__(self,
        ...
        self.cross_mamba = crossmodal( dim=config['hidden_dim'],proj_dim=config['proj_dim'])
    ...
    def get_contrastive_loss(self, image_feat, text_feat, image_feat_m, text_feat_m, alpha=0):
        ...
        loss_ita = (loss_i2t + loss_t2i +  (loss_i2i+loss_t2t)/2 ) / 2
    ...
    def get_matching_loss(self, image_embeds, image_atts, image_feat, text_embeds, text_atts, text_feat):
        ...
        with torch.no_grad():
            sim_i2t = image_feat @ text_feat.t() / self.temp +1e-8
            sim_t2i = text_feat @ image_feat.t() / self.temp+1e-8
        ...
        #                                     )
         # 拼接正负样本，得到总样本数 3B
        image_embeds_all = torch.cat([image_embeds, image_embeds_neg, image_embeds_neg], dim=0)  # [3B, L_I, D]
        text_embeds_all = torch.cat([text_embeds, text_embeds_neg, text_embeds_neg], dim=0)      # [3B, L_T, D]
         
        # vl_embeddings = torch.cat([output_pos.last_hidden_state[:, 0, :], output_neg.last_hidden_state[:, 0, :]], dim=0)
        # vl_output = self.itm_head(vl_embeddings)
       
        fused_all = self.cross_mamba(image_embeds_all, text_embeds_all)  # [3B, D]

    
        # 计算匹配logits
        itm_logits = self.itm_head(fused_all)  # [3B, 2]
 
    
        # 生成标签（正样本为1，负样本为0）
        itm_labels = torch.cat([torch.ones(bs, dtype=torch.long), torch.zeros(2*bs, dtype=torch.long) ]).to(image_embeds.device)
...
def build_visual_encoder(model='vit', img_size=256, load_params=True, url=None):
    ...
    if model == 'vit':
        ...
        visual_encoder = MambaImageEncoder(img_size=img_size, dim=vision_width, depth=12)
...
def build_text_encoder(text_config='bert-base-uncased', bert_config='configs/config_bert.json'):
    ...
    # text_encoder = MambaTextEncoder(vocab_size=30522, dim=768, depth=6)
    # text_width = dim
    return text_encoder, text_width


def build_itm(input_dim: object, output_dim: object) -> object:
...
class MambaTextEncoder(nn.Module):
    """Mamba文本编码器，替代原BERT"""
    def __init__(self, vocab_size, dim=768, depth=6):
        super().__init__()
