...
from qmodel.vision.vit import interpolate_pos_embed
from qmodel.tokenization_bert import BertTokenizer
import qmodel.utils as utils
from qmodel.dataset.utils import save_result
from qmodel.dataset import create_dataset, create_sampler, create_loader, vqa_collate_fn
from qmodel.utils import cosine_lr_schedule
...
def train(model, data_loader, optimizer, epoch, device, config):
    ...
    metric_logger.add_meter('loss', utils.SmoothedValue(window_size=1, fmt='{value:.8f}'))
...
def evaluation(model, data_loader, device, config):
    ...
    result = []
    # print(f"First answer structure: {data_loader.dataset.answer_list[0]}")
    # answer_list = [answer.get('ans', '')  + config['eos'] for answer in data_loader.dataset.answer_list]
    answer_list = [answer + config['eos'] for answer in data_loader.dataset.answer_list]

    # (f"First answer structure: {data_loader.dataset.answer_list[0]}")
...
def main(args, config):
    ...
    for epoch in range(start_epoch, config['max_epoch']):
        ...
        if utils.is_main_process():
            ...
            if args.is_save_path and epoch > 1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_use', default='tcm', help='choose medical vqa dataset(rad, pathvqa, slake)')
    parser.add_argument('--is_save_path', default=True)
    parser.add_argument('--checkpoint',default='D:/Fusion/output/pretrain/2025-04-30_22-36/med_pretrain_49.pth')
    parser.add_argument('--output_suffix', default='', help='output suffix, eg. ../rad_29_1')
    parser.add_argument('--output_dir', default='D:/Fusion/output/vqa',help='the final output path, need not to assign')
    ...
    args.output_dir = 'output/vqa/' + args.dataset_use + args.output_suffix

    # config = yaml.load(open('./configs/VQA.yaml', 'r'), Loader=yaml.Loader)

    yaml = YAML.YAML(typ='rt')
    with open('./configs/VQA.yaml', 'r',encoding='utf-8') as file:
